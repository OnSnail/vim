...
cd /Users/wrb/goworkspace/src/github.com/wuranbo/avro/lang/py
vim --cmd "set path+=~/goworkspace/src/github.com/wuranbo/avro/lang/py" \
  --cmd "autocmd BufNewFile,BufRead *.tpl set filetype=html" \
  --cmd "autocmd BufNewFile,BufRead *.sample set filetype=dosini" \
  -c "nnoremap <silent> <F5>  :<C-u>Unite -smartcase -buffer-name=files -start-insert file_rec/git:/Users/wrb/github/wuranbo/avro/lang/py:<CR>" \
  -c "nnoremap <F3>  :<C-u>Unite -smartcase -start-insert grep:/Users/wrb/github/wuranbo/avro/lang/py:--python<CR>" \
