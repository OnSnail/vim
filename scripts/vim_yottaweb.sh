...
vim --cmd "set path+=~/yottaweb" \
  --cmd "set wrap" \
  -c "nnoremap <silent> <F5>  :<C-u>Unite -smartcase -buffer-name=files -start-insert file_rec/git:/Users/wrb/yottabyte/src/YottaWeb/:--full-name<CR>" \
  ...
  -c "au FileType html nnoremap <buffer> <F3>  :<C-u>Unite -smartcase -start-insert grep:/Users/wrb/yottabyte/src/yottaweb/yottaweb/templates:--html<CR>" \
